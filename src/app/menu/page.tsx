      ...
      {menu.map((category) => (
        <Link href={`/menu/${category.slug}`} key={category.id} className='w-full h-1/3 bg-cover p-8 md:h-1/2' style={{backgroundImage: `url(${category.img})`}}>
          ...
          <div className={`text-${category.color} w-1/2`}>
            ...
            <button className='hidden md:block bg-gray-800 py-2 px-4 hover:bg-gray-700 mt-5 cursor-pointer rounded-xl text-white transition-all duration-500'>Submit</button>
          </div>
        </Link>
      ))}
    </div>
